...
const DetailFruit = () => {
    ...
    const [errorTextKomentar, setErrorTextKomentar] = useState('')
    ...
    const handleCriateKomentar = () => {
        if (!textKomentar) {
            setErrorTextKomentar('Komentar is required.')
            return
        }

        const data = {
            comment_content: textKomentar,
            fruit_id: id
        }
        axios.post(`${process.env.REACT_APP_API_URL}/api/reply-komentar`, data, { headers: { Authorization: `Bearer ${token}` } })
            .then(() => {
                setTextKomentar('')
                axios.get(`${process.env.REACT_APP_API_URL}/api/reply-komentar/${id}`)
                    .then((result) => {
                ...
                <div>
                    ...
                    <div>
                        <label className="text-black">
                            <p className="font-bold">Beri Komentar</p>
                            <textarea onChange={(e) => {
                                setTextKomentar(e.target.value)
                                setErrorTextKomentar('')
                            }} id="w3review" name="w3review" rows="4" cols="50" className="border w-full" />
                            {errorTextKomentar && <p className="text-red-600">{errorTextKomentar}</p>}
                        </label>
                        <button onClick={() => handleCriateKomentar()} className="p-2 bg-[#3A5B22] rounded w-full text-white">Kirim</button>
                    </div>
                </div>
